...
def _normalize_slash(lines, slash='unix'):
    ...
    newlines = []
    for line in lines:
        line = line.replace(EOL_WINDOWS, EOL_UNIX).replace(EOL_MAC, EOL_UNIX)
        counts = repr(line).count(SLASH_WINDOWS)
        if line.endswith(b"\\\r\n") or line.endswith(b"\\\n"): counts=counts - 1
        line = eval(repr(line).replace(SLASH_WINDOWS, SLASH_UNIX, counts))
...
def main():
    parser = argparse.ArgumentParser(
        prog='crlf',
        description='Replace backslash (windows) with slash (unix).')
    ...
    for file_to_process in files_to_process:
        ...
        if os.path.isfile(file_to_process):
            ...
            if args.windows is True:
                newdata = _normalize_slash(data, slash='windows')
            else:
                newdata = _normalize_slash(data, slash='unix')

            if newdata != data:
                if args.quiet is False:
                    if args.windows is True:
                        if args.dryrun is True:
                            print("+ '{0}' : slash would be replaced with backslash"
                                  .format(file_to_process))
                        else:
                            print("+ '{0}' : replacing slash with backslash"
                                  .format(file_to_process))
                    else:
                        if args.dryrun is True:
                            print("+ '{0}' : backslash would be replaced with slash"
                                  .format(file_to_process))
                        else:
                            print("+ '{0}' : replacing backslash with slash"
                                  .format(file_to_process))
